...
def buscoSimbolo_huawei(linea):
    if linea.find("!=") != -1:
        ...
        print(f"Error de símbolo en {linea}")
        simbolo = ""
    return simbolo
...
def proxSeparador_nokia(condicion):
    if min(condicion.find("&&"), condicion.find("||")) != -1:
        return min(condicion.find("&&"), condicion.find("||"))
    elif condicion.find("&&") != -1:
        return condicion.find("&&")
...
def armadoCondiciones_nokia(condStr):
    condStr = condStr.replace("AND", "&&").replace("OR", "||").replace(" ", "")
    clave = condStr
    #condStr = condStr.replace(" ", "")
    condList=[]
    #for algo in condStr:
    if condStr.find("&&") == -1 and condStr.find("||") == -1:
        ...
        #print(condStr)
        for algo in condStr:
            separadorIndice = proxSeparador_nokia(condStr)
            if separadorIndice != -1:
                condList.append(condStr[:separadorIndice].strip().replace("(","").replace(")",""))
                condStr = condStr[separadorIndice+2:].strip()
            else:
    ...
    return [clave, condList]

def buscoSimbolo_nokia(linea):
    if linea.find("<>") != -1:
        simbolo = "<>"
    elif linea.find(">") != -1:
        simbolo = ">"
...
def resumen_mo_nokia(full_data):
    df_temp = full_data["MO Class"].drop_duplicates()
    dic_temp = {}
    for label, content in df_temp.items():
        ...
        if mo_full.rfind("/") == -1:
            ...
            mo_res = mo_full[mo_full.rfind("/") + 1:]
        
        dic_temp.update({mo_res: mo_full})
    return dic_temp
...
def consistencia_ADRN(vendor, tecnologia, template):
    ...
    if vendor == "H":
        ...
        mo_traductor = resumen_mo_nokia(df)
        ...
        for algo in respuesta_data:
            ...
            if algo.get("condition") != None and algo.get("error") == None:
                ...
                condiciones_list = armadoCondiciones_nokia(condicion_temp)
                condiciones = condiciones_list[1]
                condicion_clave = condiciones_list[0]
                #print(f"Debo resolver la condición {condiciones}")
                """ 
                ...
                for condicion in condiciones:
                    ...
                    simbolo = buscoSimbolo_nokia(condicion)
                    if condicion.find(".") != -1:
                        paramMo_cond = condicion[:condicion.find(".")]
                    else:
                        paramMo_cond = paramMo
                    if paramMo_cond in mo_traductor.keys():
                        paramMo_cond = mo_traductor[paramMo_cond]
                    paramName_cond = condicion[condicion.find(".") + 1 : condicion.find(simbolo)]
                    paramVal_cond = condicion[condicion.find(simbolo) + len(simbolo):]
                    
                    #if df[(df["MO"] == paramMo_cond) & (df["Parameter ID"] == paramName_cond)].empty:

                    if df[(df["MO Class"] == paramMo_cond) & (df["Abbreviated Name"] == paramName_cond) & (df["Parent Structure"] != df["Parent Structure"])]["Data Type"].empty and df[(df["MO Class"] == paramMo_cond) & (df["Parent Structure"] + df["Abbreviated Name"] == paramName_cond)].empty:
                        #print(f"{paramMo_cond} - {paramName_cond}: no existe")
                        print(f"La condición {condicion} no es válida, no se encuentra la combinación MO Class-Abbreviated Name")
                        condicionesErradas += 1
                        algo.update({"error": True})
                        #flagOk = False
                    else: 
                        if not df[(df["MO Class"] == paramMo_cond) & (df["Abbreviated Name"] == paramName_cond) & (df["Parent Structure"] != df["Parent Structure"])]["Data Type"].empty:
                            #print(f"Condición {condicion} es válida SIN parent")
                            #print(f"{paramMo_cond}.{paramName_cond} {simbolo} {paramVal_cond}")
                            
                            paramType_cond = df[(df["MO Class"] == paramMo_cond) & (df["Abbreviated Name"] == paramName_cond)]["Data Type"].values[0]
                            paramRange_cond = df[(df["MO Class"] == paramMo_cond) & (df["Abbreviated Name"] == paramName_cond)]["Range and step"].values[0]
                            formulaGUI_cond = df[(df["MO Class"] == paramMo_cond) & (df["Abbreviated Name"] == paramName_cond)]["Formula for Getting Internal Value"].values[0]
                            #condicion_rearmada = f"{paramMo_cond}.{paramName_cond} {simbolo} {paramVal_cond}"
                            #condicion_clave.replace(condicion, condicion_rearmada)
                            
                        elif not df[(df["MO Class"] == paramMo_cond) & (df["Parent Structure"] + df["Abbreviated Name"] == paramName_cond)].empty:
                            #print(f"Condición {condicion} es válida CON parent")
                            
                            paramType_cond = df[(df["MO Class"] == paramMo_cond) & (df["Parent Structure"] + df["Abbreviated Name"] == paramName_cond)]["Data Type"].values[0]
                            paramRange_cond = df[(df["MO Class"] == paramMo_cond) & (df["Parent Structure"] + df["Abbreviated Name"] == paramName_cond)]["Range and step"].values[0]
                            formulaGUI_cond = df[(df["MO Class"] == paramMo_cond) & (df["Parent Structure"] + df["Abbreviated Name"] == paramName_cond)]["Formula for Getting Internal Value"].values[0]
                        
                        #print(f"{paramMo_cond}.{paramName_cond} {simbolo} {paramVal_cond}")
                        #def internal_value_nokia(paramType, paramRange, guiValue, formulaInternal):
                        
                        paramVal_cond_internal = internal_value_nokia(paramType_cond, paramRange_cond, paramVal_cond, formulaGUI_cond)
                        condicion_rearmada_gui = f"{paramMo_cond}.{paramName_cond} {simbolo} {paramVal_cond}"
                        condicion_rearmada_internal = ""
                        
                        if paramVal_cond_internal == "error":
                            print(f"No se pudo resolver el valor interno de la condición {condicion_rearmada_gui}")
                            algo.update({"error": True})
                            condicionesErradas += 1
                        else:
                            condicion_rearmada_internal = f"{paramMo_cond}.{paramName_cond} {simbolo} {paramVal_cond_internal}"
                        
                        condicion_clave = condicion_clave.replace(condicion, condicion_rearmada_internal)
                
                condicion_clave = condicion_clave.replace("&&", " AND ").replace("||", " OR ")
                algo.update({"condition_full": condicion_clave})
            
            if algo.get("error") == None:
                respuesta_data_ok.append(algo)

    print(f"Fin del análisis. Se verificaron {parametros} parámetros, encontrando {parametrosErrados + condicionesErradas} errores ({parametrosErrados} parámetros y {condicionesErradas} condiciones)")
